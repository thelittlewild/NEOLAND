...
const toggleFavChampion = async (req, res, next) => {
	try {
		...
		setTimeout(async () => {
			return res.status(200).json(await User.findById(_id).populate('championsFav'));
		}, 100);
	} catch (error) {
		return next(error);
	}
...
const toggleFavSkin = async (req, res, next) => {
	try {
		...
		setTimeout(async () => {
			return res.status(200).json(await User.findById(_id).populate('skinsFav'));
		}, 100);
	} catch (error) {
		return next(error);
	}
...
const userTopSkinOwner = async (req, res, next) => {
	try {
		...
		return res.status(400).json('error al mostrar el top 10 Jugadores con más skins');
	}
};
